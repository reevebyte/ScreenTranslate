...
`screentrans/updater.py` 和 `screentrans/ui/update_dialog.py` 会检查版本，并允许用户直接下载
正式安装器。清单限制为 256 KiB，安装包限制为 512 MiB；初始地址必须是 GitHub Release 的
`update-manifest.json`，重定向只能留在 GitHub 的资产基础设施中。返回的发布页面及安装包必须
精确匹配内置仓库、`v<version>` 标签和 `ScreenTranslate-<version>-setup-x64.exe` 文件名。
下载采用流式临时文件，超过清单大小、实际大小不符、SHA-256 不符、取消或中断都会拒绝安装并
清理未完成文件。再次下载时还会清理旧版本缓存和残留 `.part` 文件，避免安装包逐版本累积。
已验证的当前版本缓存可以复用，但安装前辅助进程还会重新校验路径、大小和 SHA-256；缓存目录、
版本目录和安装包本身如果是符号链接或 Windows 重解析点，也会被拒绝。

校验成功不等于静默安装：更新窗口会再次询问用户，用户确认后主程序才启动独立辅助模式并退出；
辅助模式等待旧进程结束，再用 Windows 正常方式启动安装器。项目不使用 Authenticode 证书，
所以 Windows 仍可能显示“未知发布者”，应用不会尝试绕过该提示。用户也可以点击“发布说明”在
系统浏览器中打开同一官方仓库并手动下载。
